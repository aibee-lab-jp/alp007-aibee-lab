...
- **【暫定】自社アプリの固有名を画面に出さない（決定・2026-08-31）**：とりあえず47 の公開前に本サイトを先行リリースするため、アプリ名・ポータルへのリンク・役割分担の案内を画面から外す。`NEXT_PUBLIC_PORTAL_URL` は Terraform・`env.ts` に残置し（UI から参照しないだけ）、公開時の復帰を容易にする。戻す箇所の一覧はブリーフ §4 冒頭。**見直す合図：とりあえず47 の公開時**（§11 の残タスク）。
...
- **ページ文言の正は `src/lib/site-content.ts`（コード）**。文言はここに集約し、言い回しの調整はコードの修正だけで完結する（仕様書・ブリーフの改訂は不要）。理由：1語の修正のたびに仕様書の改訂を要求すると、いずれ改訂されないまま仕様書が古びる。**ただし判断を伴う決定（屋号表記・所在地・沿革の事実・禁止語・代表名や電話番号の非掲載・実績の書き方・privacy の必須記載事項・ページやセクションの構成）を変えるときは、仕様書と `SITE_DESIGN_BRIEF.md` の改訂が要る**（制約の一覧はブリーフ §4 冒頭）。ブリーフ §4〜§6 は初版の参考スナップショットとして残す。
...
- **【暫定】役割分担の案内は画面に出さない**：本来はフォーム冒頭に「アプリの不具合・ご要望は とりあえず47 のお問い合わせへ」の誘導を置く設計だが、自社アプリの固有名を伏せている間（§1 の暫定決定）は外す。復帰時にリンク先を `NEXT_PUBLIC_PORTAL_URL` から組み立てる。
- **カテゴリ選択は設けない（決定）**：受け付けるのはアイビーラボ宛の連絡のみで、分類の必要がないため（アプリの不具合・要望は、公開後は冒頭の誘導が just47 へ流す）。just47 実装の流用時はカテゴリ関連を削る（CLAUDE.md の差分ホットスポット）。
...
7. **とりあえず47 の公開時**：伏せている自社アプリの固有名・リンク・役割分担の案内を戻す（§1 の暫定決定。戻す箇所はブリーフ §4 冒頭。`NEXT_PUBLIC_PORTAL_URL` は残置済みのため UI の復帰のみ）。沿革2026 の行も「開始」→「公開」へ差し替える
...
| 2026-08-31 | 第15版 | **文言の正をコード（`src/lib/site-content.ts`）へ移した**（§4）：言い回しの調整に仕様書の改訂を不要とし、代わりに「変えるには改訂が要る制約」の一覧をブリーフ §4 冒頭に新設。理由は、1語の修正のたびに改訂を求めるとルールが形骸化し仕様書が古びるため。**【暫定】自社アプリの固有名を画面に出さない決定**を §1 に追加（とりあえず47 の公開前に本サイトを先行リリースするため。`NEXT_PUBLIC_PORTAL_URL` は残置し復帰を容易に）。§11 に公開時の復帰タスクを追加。実績の8件への拡充はブリーフ第5版に記録。**§5 の役割分担の案内も「暫定的に外す」へ追随**し、カテゴリ不採用の理由を誘導の有無に依存しない形に書き換え（Claude Code の指摘による） |
