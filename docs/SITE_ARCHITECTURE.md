...
| 2026-08-31 | 第15版 | **文言の正をコード（`src/lib/site-content.ts`）へ移した**（§4）：言い回しの調整に仕様書の改訂を不要とし、代わりに「変えるには改訂が要る制約」の一覧をブリーフ §4 冒頭に新設。理由は、1語の修正のたびに改訂を求めるとルールが形骸化し仕様書が古びるため。**【暫定】自社アプリの固有名を画面に出さない決定**を §1 に追加（とりあえず47 の公開前に本サイトを先行リリースするため。`NEXT_PUBLIC_PORTAL_URL` は残置し復帰を容易に）。§11 に公開時の復帰タスクを追加。実績の8件への拡充はブリーフ第5版に記録 |
