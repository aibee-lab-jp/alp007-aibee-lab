...
- 見出し・本文とも Noto Sans JP（明朝は使わない。ブリーフ第4版で廃止）。
- 配色・トーンは本サイト固有（`docs/SITE_DESIGN_BRIEF.md` §2 に従う）。純白寄りの地・差し色はディープネイビー `#24405E`・ヘッダーは淡いブルーグレー。just47 の中立スタイルや「ど忘れ県」の古地図テーマを持ち込まない。
- **ページ文言の正は `src/lib/site-content.ts`（コード）**。`docs/SITE_DESIGN_BRIEF.md` §4〜§6 は初版の参考スナップショットであり、正ではない（食い違ったらコードが現在の姿）。
- ただし**判断を伴う決定は勝手に変えない**：屋号表記・所在地・沿革の事実・使わない語（「会社概要」等）・代表名や電話番号の非掲載・実績の書き方（社名を出さない／AWS 固有名は平易に言い換える）・フォームの項目構成・privacy の必須記載事項・ページやセクションの構成。制約の一覧はブリーフ §4 冒頭。変更が必要ならオーナーへ報告すること。
